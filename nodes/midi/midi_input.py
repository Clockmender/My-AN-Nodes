...
class MidiInputNode(bpy.types.Node, AnimationNode):
    ...
    def create(self):
        ...
        self.newOutput("Integer List", "Note Idices", "indV")
    ...
    def execute(self):
        ...
        indV = [item.noteIndex for item in self.notes]
        notes_s = [item.noteName.split(' ')[2].split('_')[1]+'_'+self.suffix for item in self.notes]
        values = [item.value for item in self.notes]
        bpm_int = int(self.bpm)
        nn_int = self.nn
        dd_int = self.dd
        ot_off = self.offset
        return notes, notes_s, DoubleList.fromValues(values), indV, bpm_int, nn_int, dd_int, ot_off
