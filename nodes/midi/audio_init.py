...
class AudioSDNode(bpy.types.Node, AnimationNode):
    ...
    def draw(self,layout):
        if len(self.devINames) > 0:
            ...
            layout.label(text="No Input Devices:", icon = "ERROR")
        if len(self.devONames) > 0:
            layout.prop(self, "modeO")
        else:
            layout.label(text="No Output Devices:", icon = "ERROR")
        layout.prop(self, "multI")
        layout.prop(self, "runSD")
        if self.message != '':
            layout.label(text=self.message, icon = "INFO")
        layout.label(text='Number of Input Channels: '+str(self.chans), icon = "INFO")
