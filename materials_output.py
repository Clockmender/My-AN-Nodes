...
class materialsOutputNode(bpy.types.Node, AnimationNode):
    ...
    slot = IntProperty(name = "Material Slot", default = 0)
    ...
    def draw(self, layout):
        layout.prop(self, "trip")
        layout.prop(self, "slot")

    def execute(self, obj, inpt, trip, mat_1, mat_2):
        if mat_1:
            if len(obj.material_slots) < 1:
                obj.data.materials.append(mat_1)
            elif not mat_2:
                if len(obj.material_slots) >= (self.slot + 1):
                    obj.material_slots[self.slot].material = mat_1
            elif mat_2 and (inpt > trip):
                if len(obj.material_slots) >= (self.slot + 1):
                    obj.material_slots[self.slot].material = mat_2
            elif mat_2 and (inpt <= trip):
                if len(obj.material_slots) >= (self.slot + 1):
                    obj.material_slots[self.slot].material = mat_1
